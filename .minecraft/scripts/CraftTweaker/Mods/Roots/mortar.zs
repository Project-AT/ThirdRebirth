...
function mortarUtils(output as IItemStack, input as IIngredient[], isRemove as bool) {
    if (isRemove) Mortar.removeRecipe(output);
...
mortarUtils(<roots:spell_dust>.withTag({spell_storage: {s: "roots:spell_chrysopoeia"}}), [
...
for i in 0 to 16 {
    ...
    mortarUtils(<botania:petal>.definition.makeStack(i) * 6, [doubleflower], false);
    mortarUtils(<botania:petal>.definition.makeStack(i) * 3, [<botania:flower>.definition.makeStack(i)], false);
}